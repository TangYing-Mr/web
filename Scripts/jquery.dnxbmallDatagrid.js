...
(function ($) {
    ...
    function request(target, param) {
        var opts = $.data(target, "dnxbmallDatagrid").options;
        if (param) {
        ...
        //显示加载信息
        $(target).dnxbmallDatagrid("loading");
        ...
        function ajaxRequest() {
            $.ajax({
                ...
                success: function (data) {
                    setTimeout(function () {
                        $(target).dnxbmallDatagrid("loaded");
                    }, 0);
    ...
    function getSelectRows(target) {
        var opts = $.data(target, "dnxbmallDatagrid").options;
        var data = $.data(target, "dnxbmallDatagrid").data;
        var tBody = $(target).find("tbody");
        if (opts.idField) {
            return $.data(target, "dnxbmallDatagrid").selectedRows;
        } else {
            var rowAry = [];
            $("tr.dnxbmallDatagrid-row-selected", tBody).each(function () {
                var row_index = parseInt($(this).attr("dnxbmallDatagrid-row-index"));
    ...
    function selectAll(target) {
        var opts = $.data(target, "dnxbmallDatagrid").options;
        var data = $.data(target, "dnxbmallDatagrid").data;
        var selectRows = $.data(target, "dnxbmallDatagrid").selectedRows;
        var tBody = $(target).find("tbody");
        var rows = data.rows;
        selectRows = [];
        $("div.dnxbmallDatagrid-cell-check input[type=checkbox]:enabled", tBody)
            .prop("checked", true).parents('tr').addClass("dnxbmallDatagrid-row-selected");
        for (var i = 0; i < rows.length; i++) {
            if (opts.idField) {
                var row = rows[i];
                if ($("div.dnxbmallDatagrid-cell-check input[type=checkbox]").eq(i).is(':checked')) {
                    selectRows.push(row);
                }
            }
        }
        $.data(target, "dnxbmallDatagrid").selectedRows = selectRows;
    };
    function clearSelectRows(target) {
        ...
        var selectRows = $.data(target, "dnxbmallDatagrid").selectedRows;
        var tBody = $(target).find("tbody");
        $("tr", tBody).removeClass("dnxbmallDatagrid-row-selected");
        $("div.dnxbmallDatagrid-cell-check input[type=checkbox]", tBody).prop("checked", false);
    ...
    function selectRow(target, index) {
        ...
        var tr = $("tr[dnxbmallDatagrid-row-index=" + index + "]", tBody);
        if (!tr.hasClass("dnxbmallDatagrid-row-selected")) {
            tr.addClass("dnxbmallDatagrid-row-selected");
            var ck = $("div.dnxbmallDatagrid-cell-check input[type=checkbox]", tr);
    ...
    function unSelectRow(target, index) {
        ...
        var tBody = $(target).find("tbody");
        var tr = $("tr[dnxbmallDatagrid-row-index=" + index + "]", tBody);
        var ck = $("tr[dnxbmallDatagrid-row-index=" + index + "] div.dnxbmallDatagrid-cell-check input[type=checkbox]", tBody);
        tr.removeClass("dnxbmallDatagrid-row-selected");
    ...
    function bindCellsEvents(target) {
        var opts = $.data(target, "dnxbmallDatagrid").options;
        var total = $.data(target, "dnxbmallDatagrid").data.total;
        ...
        $("div.dnxbmallDatagrid-cell-check input[type=checkbox]", $(target)).unbind("click").bind("click", function (e) {
            var findIndex = $(this).parent().parent().parent().attr("dnxbmallDatagrid-row-index");
            if (opts.singleSelect) {
                clearSelections(target);
    ...
    function loadData(target, _data) {
        ...
        $.data(target, "dnxbmallDatagrid").data = _data;
    ...
    //插件构造函数
    $.fn.dnxbmallDatagrid = function (options, param) {
        if (typeof options == "string") {
            return $.fn.dnxbmallDatagrid.methods[options](this, param);
        }

        options = options || {};
        return this.each(function () {
            var state = $.data(this, "dnxbmallDatagrid");
            var opts;
            if (state) {
                opts = $.extend(state.options, options);
                state.options = opts;
            } else {
                opts = $.extend({}, $.fn.dnxbmallDatagrid.defaults, options);
                //console.log(opts);
                $.data(this, "dnxbmallDatagrid", {
                    options: opts,
                    selectedRows: [],
                    data: {
                        total: 0,
    ...
    //插件对外公开的所有方法集合
    $.fn.dnxbmallDatagrid.methods = {
        options: function (jq) {
            var opts = $.data(jq[0], "dnxbmallDatagrid").options;
            return opts;
        },
        data: function (jq) {
            var data = $.data(jq[0], "dnxbmallDatagrid").data;
            return data;
        },
        reload: function (jq, param) {
            return jq.each(function () {
                if (!param.pageNumber) param.pageNumber = 1;
                $.data(this, "dnxbmallDatagrid").options.pageNumber = param.pageNumber;
                request(this, param);
            });
        },
        ...
        loading: function (jq) {
            return jq.each(function () {
                var opts = $.data(this, "dnxbmallDatagrid").options;
        ...
        getRows: function (jq) {
            return $.data(jq[0], "dnxbmallDatagrid").data.rows;
        },
    ...
    var renderView = {
        render: function (target, container) {
            var opts = $.data(target, "dnxbmallDatagrid").options;
            var rows = $.data(target, "dnxbmallDatagrid").data.rows;
            var closFileds = $(target).dnxbmallDatagrid("getColumnFields");
            ...
            if (rows == null || rows.length <= 0) {
                ...
                var rowData = {};
                for (var i = 0; i < rows.length; i++) {
                    var cls = (i % 2 && opts.striped) ? "class=\"dnxbmallDatagrid-row-alt\"" : "";
                    ...
                    if (opts.childField && rows[i][opts.childField].length > 0) {
                        for (var j = 0; j < rows[i][opts.childField].length ; j++) {
                            ...
                            htmlAry.push("<tr dnxbmallDatagrid-row-index=\"" + i + "\" " + cls + " " + strStyle + " ");
        ...
        renderHead: function (target, container) {
            ...
            for (var i = 0; i < closFileds.length; i++) {
                ...
                var content = "",
                	...
                	col = $(target).dnxbmallDatagrid("getColumnOption", findFiled),
                	sort = '',
                	sortHtml = '',
        ...
        renderRow: function (target, fileds, rowIndex, rowData) {
            ...
            for (var i = 0; i < fileds.length; i++) {
                ...
                var col = $(target).dnxbmallDatagrid("getColumnOption", findFiled);
                if (col) {
                    var className = "";
                    ...
                    if (col.checkbox) {
                        cc.push(" class=\"dnxbmallDatagrid-cell-check ");
                    } else {
                        cc.push(" class=\"dnxbmallDatagrid-cell ");
                    }
                    cc.push("\">");
                    if (col.checkbox) {
                        if (col.formatter) {
