...
(function($) {
	$.fn.dnxbmallDistrict = function(options) {
		var defaults = {
		...
		this.each(function() {
			...
			var _this = $(this);
			var district=_this.siblings('.dnxbmall-district');
			
			if (!(params.id.length > 0)) {
				return false;
			}
			var thisId = 'dnxbmallDistrict-' + params.id;
			...
			var dnxbmallDistrict = '<div class="dnxbmall-district" id="'+thisId+'"><div class="district-hd"><span>请选择省</span></div>'+provinceStr+'<ul class="district-ul city-ul cl"></ul><ul class="district-ul county-ul cl"></ul><ul class="district-ul street-ul cl"></ul></div>';
			...
			if(params.renderTo.find("#"+thisId).length>0){
				...
				params.renderTo.append(dnxbmallDistrict);
				_this.addClass('active');
			}
			...
			params.renderTo.on('click','.province-ul a',function() {
				...
				if(parent.length>0){
					...
					$(this).parents('.dnxbmall-district').hide();
					params.closeFn();
				}
			});
