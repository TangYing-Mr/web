...
; (function ($) {
    $.fn.dnxbmallSku = function (options) {
        ...
        var _this = $(this),
			...
			skuId = [0, 0, 0],
			checkTodo = function (el, data, skuId) {
			    el.each(function () {
            ...
            success: function (data) {
                ...
                if (skuLen == 2) {
                    _this.on('click', '.enabled', function () {
                        skuId[parseInt($(this).attr('st'))] = $(this).attr('cid');
                        checkTodo($(this).parents(params.spec).siblings(params.spec).find(params.itemClass), skuArr, skuId);
                    });
                }
                ...
                if (skuLen == 3) {
                    _this.on('click', '.enabled', function () {
                        ...
                        if (sibOne.length > 0) {
                            ...
                            checkTodo(sibSku.eq(1).find(params.itemClass), skuArr, skuId);
                        }

                        if (sibTwo.length > 0) {
                            skuId[parseInt(sibTwo.attr('st'))] = sibTwo.attr('cid');
                            checkTodo(sibSku.eq(0).find(params.itemClass), skuArr, skuId);
                        }
                    });
                }
