    ...
    $.post("/product/GetProductActives", { shopid: $('#shopid').val(), productId: gid }, function (data) {
        if (parseFloat(data.freeFreight) > 0) {
            $("#summary-promotion").append("<div class='dt l l01'>促销</div>" +
                    "<div class='promotion-l' style='float:left;width:440px;'>" +
                            "<div style='margin-bottom:5px;'><em class='hl_red_bg'>满免</em><em class='hl_red'>单笔订单满<span>" + data.freeFreight + "</span>英镑免运费</em></div>" +
                    "</div>");
        }
        if (data.ProductBonus != undefined && data.ProductBonus != null && typeof data.ProductBonus.GrantPrice != "undefined") {
            var html = '<div class="dd d02" style="padding:0;margin:0; margin-bottom:5px;"><em class="hl_red_bg">红包</em><em class="hl_red">满<span>' + data.ProductBonus.GrantPrice + '英镑</span>送红包（' + data.ProductBonus.Count + '个' + data.ProductBonus.RandomAmountStart + '—' + data.ProductBonus.RandomAmountEnd + '英镑代金券红包）</em></div>';
        ...
        if (data.FullDiscount != undefined && data.FullDiscount != null) {
			...
			$(data.FullDiscount.Rules).each(function (index, item) {
                html += '满<span>' + item.Quota.toFixed(2) + '英镑</span>减' + item.Discount.toFixed(2) + (index == data.FullDiscount.Rules.length - 1 ? '' : '，');
            });
            html += '</em></div>';
            if ($(".promotion-l").length > 0) {
                $(html).appendTo(".promotion-l");
        ...
        success: function (data) {
            ...
            if (data && data.isSelf == true) {
                ...
                if (data.cashDeposits > 0) {
                    html += '<dl class="pop-money"><dt>资质：</dt><dd><span title="该卖家已缴纳保证金' + data.cashDeposits + '英镑">' + data.cashDeposits + '英镑</span></dd></dl>'
                }
...
function checkLogin(callBack) {
    var memberId = $.cookie('Dnxbmall-User');
    if (memberId) {
        callBack();
    }
    else {
