            ...
            $.get('/shop/GetCitySubRegions?regionId=' + regionId, function (data) {
                ...
                if (data && data.regions && data.regions.length > 0) {
                    ...
                    $('.districtSelect', dialog).html(str).change(function () {
                        if (!this.value) {
                            $('.list', dialog).dnxbmallDatagrid('reload', { regionId: regionId, getParent: true });
                        } else {
                            $('.list', dialog).dnxbmallDatagrid('reload', { regionId: this.value, getParent: false });
                        }
                    });
                }
            });

            $(".list", dialog).dnxbmallDatagrid({
